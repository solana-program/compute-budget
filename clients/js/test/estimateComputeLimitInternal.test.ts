...
describe('estimateComputeUnitLimit', () => {
    ...
    it('replaces the existing set compute unit limit instruction when one exists', () => {
        ...
        const transactionMessage = {
            ...mockTransactionMessage,
            instructions: [mockInstruction, getSetComputeUnitLimitInstruction({ units: 1234 }), mockInstruction],
            lifetimeConstraint: MOCK_BLOCKHASH_LIFETIME_CONSTRAINT,
        };
    ...
    it('throws with the cause when simulation fails', async () => {
        expect.assertions(1);
        const simulationError = new SolanaError(SOLANA_ERROR__INSTRUCTION_ERROR__INSUFFICIENT_FUNDS, { index: 42 });
        ...
        await expect(estimatePromise).rejects.toThrow(
            new SolanaError(SOLANA_ERROR__TRANSACTION__FAILED_TO_ESTIMATE_COMPUTE_LIMIT, { cause: simulationError }),
        );
    });
});
